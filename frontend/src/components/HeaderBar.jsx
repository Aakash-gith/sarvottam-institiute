...
import { LogOut, MessageSquareMore, User, Bell, Calendar, BookOpen, Brain, TrendingUp, Flame } from "lucide-react";
...
function HeaderBar() {
  ...
  const [notifications, setNotifications] = useState([
    { id: 1, text: "New quiz available: Data Structures", time: "2 min ago", read: false },
    { id: 2, text: "Event reminder: Study Group at 5 PM", time: "1 hour ago", read: false },
    { id: 3, text: "You completed 5 quizzes this week!", time: "2 hours ago", read: true },
  ]);

  // News ticker items
  const newsItems = [
    "🎓 New Interactive Quiz System Launched - Try it now!",
    "📚 Campus Library Hours Extended until 11 PM",
    "👥 Study Group Formation Event - Join fellow students!",
    "🏆 Top performers this week: Keep up the great work!",
    "📅 Upcoming: End of semester review sessions",
  ];
  ...
  return (
    <div className="sticky top-0 z-20">
      {/* Main Header */}
      <div className="bg-gradient-to-r from-purple-900/40 via-bg-2 to-bg-1 backdrop-blur-md border-b border-white/10">
        <div className="flex items-center justify-between h-16 px-4">
          ...
          <div className="flex items-center gap-6 pl-14">
            ...
            <div className="hidden lg:flex items-center gap-3">
              <div className="flex items-center gap-2 bg-orange-500/10 px-3 py-1.5 rounded-lg border border-orange-500/20">
                ...
                <span className="text-orange-400 text-xs font-medium">{stats.streak} Day Streak</span>
              </div>
              <div className="flex items-center gap-2 bg-blue-500/10 px-3 py-1.5 rounded-lg border border-blue-500/20">
                <Brain size={14} className="text-blue-400" />
                <span className="text-blue-400 text-xs font-medium">{stats.quizCount} Quizzes</span>
              </div>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2">
            ...
            <button 
              ...
              title="Messages"
            >
              <MessageSquareMore size={20} className="text-gray-300 group-hover:text-white" />
            </button>
      ...
      {/* News Ticker */}
      <div className="bg-gradient-to-r from-purple-600/20 via-bg-1 to-purple-600/20 border-b border-white/5">
        <div className="flex items-center h-8 px-4 overflow-hidden">
          <span className="text-xs text-purple-400 font-semibold mr-3 flex-shrink-0 hidden sm:block">📢 NEWS</span>
            ...
            {newsItems.map((_, idx) => (
              <button
                ...
                className={`w-1.5 h-1.5 rounded-full transition-all ${
                  idx === currentNewsIndex ? 'bg-purple-500 w-3' : 'bg-gray-600 hover:bg-gray-500'
                }`}
              />
            ))}
          </div>
